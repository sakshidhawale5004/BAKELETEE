          ...
          <div className="fixed inset-0 md:inset-auto md:left-4 md:bottom-6 z-50 flex items-center justify-center md:items-end md:justify-start p-2 sm:p-4 md:p-0">
            <div className="w-11/12 sm:max-w-md md:max-w-sm max-h-[92vh] md:max-h-auto bg-white rounded-2xl shadow-xl border border-border overflow-hidden flex flex-col animate-fade-in">
              ...
              <div className="bg-gradient-primary p-2 sm:p-3 md:p-4 relative text-primary-foreground flex-shrink-0 rounded-t-2xl">
                ...
                <button
                  onClick={() => setIsExpanded(false)}
                  className="absolute top-2 right-2 w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 rounded-full bg-primary-foreground/20 hover:bg-primary-foreground/30 flex items-center justify-center transition-colors z-50"
                  aria-label="Close loyalty card"
                >
                  <X className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-primary-foreground" strokeWidth={3} />
                </button>

                {/* Header Content - Centered */}
                <div className="text-center pr-5">
                  <h3 className="text-sm sm:text-base md:text-lg font-bold leading-tight">Bakelette Loyalty Card</h3>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-1.5 sm:p-3 md:p-4 space-y-1.5 sm:space-y-2.5 md:space-y-3 bg-white">
                  ...
                  <div className="flex justify-center items-center gap-1 sm:gap-1.5 md:gap-2 px-1 mb-2">
                    {[1, 2, 3, 4].map((index) => (
                      <div key={index} className="flex flex-col items-center flex-shrink-0">
                        ...
                        <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-primary fill-primary text-primary-foreground" />
                      </div>
                    ))}
                    {/* 5th stamp - Empty/Next */}
                    <div className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 rounded-full border-2 border-primary/40 flex items-center justify-center bg-primary/5 flex-shrink-0">
                ...
                <div className="space-y-1 sm:space-y-1.5 mt-1.5">
                  ...
                  <button
                    onClick={() => setIsExpanded(false)}
                    className="w-full border-2 border-primary text-primary font-bold py-1 sm:py-1.5 px-3 rounded-lg transition-all hover:bg-primary/5 text-xs sm:text-sm"
                  >
                    Close
                  </button>
                </div>
